...
export const load = (async (event) => {
    ...
    for (const row of resultUsers) {
        ...
        const query = new Query(`SELECT COUNT(*) FROM gh_issues WHERE consent_interaction_rater_${index} IS NOT NULL AND resolution_rater_${index} IS NOT NULL AND privacy_issue_rater_${index} IS NOT NULL`)
